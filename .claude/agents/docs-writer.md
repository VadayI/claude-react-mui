...
- `@.claude/rules/verification.md` — `docs/verify/<feature>.md` derived from `.claude/memory/routes.json` + `docs/api/openapi.yml`
...
2. **Three-way reconciliation** — routes in `.claude/memory/routes.json` vs `docs/api/openapi.yml` vs `docs/api/INDEX.md` must agree. The OpenAPI schema is the source of truth.
3. **Verification guide** — generate `docs/verify/<feature>.md`:
