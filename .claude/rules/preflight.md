...
3. **Backend API contract reachable.** This is a frontend-only repo — it consumes a backend API. The backend's OpenAPI schema must be available so the typed client and types can be generated (`src/lib/api/openapi.yml` present, or `VITE_OPENAPI_URL` reachable to pull it). If the contract is missing or unreachable → STOP: the UI would be coded against an imagined API. (`api:pull` + `api:types`, @.claude/rules/api-client.md.)
4. **Design references (recommended).** Mockups, a component inventory, brand/theme tokens, or at least a description of look & feel, so `ui-architect` and the theme are grounded. If absent, note that the UI will follow MUI defaults and proceed.
...
- The **orchestrator** runs preflight at project kickoff, delegating access checks (Context7, GitHub, backend contract, stack deps) to `devops` and brief/stack comprehension to `ba`.
...
`/doctor` checks the **environment** (tools, services, git hygiene). Preflight checks the **inputs to build** (brief, stack, backend contract, design refs, docs/GitHub access). On a fresh machine run `/doctor` first, then preflight before the first feature.
