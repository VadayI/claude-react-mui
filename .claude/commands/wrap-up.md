...
Refresh `docs/HANDOFF.md` (the rolling snapshot read FIRST on the next machine/session) — same structure `/handoff` writes: `## Next steps` and `## Open questions` headings included, so `auditor` can parse them. Carry the _Next steps_ from step 5 and any unresolved items into _Open questions_.
