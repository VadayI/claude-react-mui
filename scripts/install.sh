...
# Usage: bash scripts/install.sh [TARGET_DIR] [--ref REF] [--url URL] [--dry-run]
# Python 3.13+ and Git required. Native Windows: explicit Git for Windows Bash.
# No app generation, active workflows, env copies, Git writes or dependency installs.
set -euo pipefail
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --ref|--url)
      ...
      if [ "$1" = --ref ]; then REF="$2"; else UPSTREAM_URL="$2"; fi
      shift 2 ;;
...
if [ "$APPLY" -eq 1 ]; then
  "$PYTHON" "$CLONE/scripts/ai/install.py" --target "$TARGET" --apply
else
  "$PYTHON" "$CLONE/scripts/ai/install.py" --target "$TARGET"
fi
printf '%s\n' "Payload verified. Read docs/ai/workflows/bootstrap.md with Claude /bootstrap or Codex bootstrap skill."
printf '%s\n' "CI activation/first push waits for P05/P06; no live workflows or secrets were seeded."
