...
3. **Contract available.** `CONTRACT_VERSION` is set (in `.env` or environment) and `src/lib/api/openapi.yml` is present. The contract is vendored from `VadayI/claude-api-contract` at that tag — run `npm run api:pull` to fetch it, then `npm run api:types` to regenerate types. The contract-sync gate (`scripts/check_contract_sync.sh`) must be GREEN before any feature pipeline starts. If the contract is missing or VERSION is unset → STOP: the UI would be coded against an imagined API. (@.claude/rules/api-client.md.)
